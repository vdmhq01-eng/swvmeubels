  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 gap-5 md:grid-cols-3">
        <KPI icon={<Icon.Calendar className="h-5 w-5" />} label="Komende week" value={`${upcoming.slice(0, 7).length} items`} />
        <KPI icon={<Icon.BookOpen className="h-5 w-5" />} label="Examens" value={`${items.filter((p) => p.type === 'EXAMEN').length}`} />
        <KPI icon={<Icon.Briefcase className="h-5 w-5" />} label="BPV periodes" value={`${items.filter((p) => p.type === 'BPV').length}`} />
      </div>
...
function KPI({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="card-padded">
      <div className="grid h-10 w-10 place-items-center rounded-xl bg-wood-50 text-wood-700 ring-1 ring-wood-100">{icon}</div>
